...
class SignUpAssertions:
    ...
    def verify_birthday_day_error(self):
        browser.element(by.xpath(SignUpErrorLocators.BIRTHDAY_DAY_ERROR_MESSAGE)).should(
            have.text(ErrorMessages.BD_INVALID_DAY_RANGE_ERROR.value))

    def verify_birthday_month_error(self):
        browser.element(by.xpath(SignUpErrorLocators.BIRTHDAY_MONTH_ERROR_MESSAGE)).should(
            have.text(ErrorMessages.BD_MONTH_REQUIRED_ERROR.value))

    def verify_birthday_year_error(self):
        browser.element(by.xpath(SignUpErrorLocators.BIRTHDAY_YEAR_ERROR_MESSAGE)).should(
            have.text(ErrorMessages.BD_INVALID_YEAR_FORMAT_ERROR.value))
