...
class TestSignUpValidation:
    ...
    def test_email_is_required(self, signup_page, test_user, assert_signup):
        ...
        with step(f'Skip email and click Next button'):
    ...
    @allure.description('Verifies password length validation rule when password is less than 10 characters')
        ...
        with step('Enter too short password (7 chars)'):
            ...
            with step('Verify: 10 chars rule is not met'):
                assert_signup.verify_password_validation_state(
                    ten_chars_met=False,
                    letter_met=True,
                    number_special_met=True
                )

    @allure.title('Password must contain at least one letter')
    @allure.description('Verifies letter requirement validation when password has no letters')
    ...
    @allure.description('Verifies number/special requirement validation when password has only letters')
    def test_password_must_have_number_or_special(self, signup_page, test_user, assert_signup):
        with step('Open signup page and navigate to password step'):
            signup_page.open_signup_page()
            signup_page.click_signup_button()
            signup_page.fill_signup_step_email(test_user['email'])
        ...
        with step('Enter password without number/special (only letters)'):
            ...
            with step('Verify: number/special rule is not met'):
                assert_signup.verify_password_validation_state(
                    ten_chars_met=True,
                    letter_met=True,
                    number_special_met=False
                )

    @allure.title('Personal info fields are required')
    @allure.description('Verifies validation errors for empty username, birthday and gender fields')
    def test_step_personal_info_required_fields(self, signup_page, test_user, assert_signup):
    ...
    @allure.description('Verifies day validation error when value is outside valid range')
